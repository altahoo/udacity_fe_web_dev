...
var resizePizzas = function(size) { 
  ...
  function changePizzaSizes(size) {
    // Optimization: 
    // Delete determinDX() and simplify the changePizzaSizes() function to avaiod FSL
    switch(size) {
        case "1":
          newWidth = 25;
          break;
    ...
    var numOfPizza = pizzaContainers.length;
...
// Optimization: moving pizzasDiv outside the loop to improve loading performance
...
//Optimization: wrap up the updatePosition() to a request animation function
var animating = false;
...
function updatePositions() {
  ...
  //Optimization: getElementsByClassName('mover') is faster then querySelectorAll(".mover");
  var items = document.getElementsByClassName('mover');

  // Optimization: reducing the loop work
  // Since there are only 5 possible values for phase, we can break the loop into two loops
  // One loop calculates the 5 phases, only 5 iterations are needed
  // The other loop calculates the position by iterating every item.
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  // Optimization: declare the elem variable outside the loop 
  // so that it is not being created every time the loop is executed.
  var elem;
  // Optimization: getElementById('movingPizzas1') is faster than querySelector("#movingPizzas1")
  // Also, move this variable's declaration outside the loop
  // Optimization:  Reduce the number of moving pizzas to 40
